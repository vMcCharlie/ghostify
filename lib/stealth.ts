...
const pointHex = (point: secp.ProjectivePoint) => `0x${bytesToHex(point.toRawBytes(false))}` as `0x${string}`;
...
export function createKeys() {
  ...
  return { spendingPrivateKey: `0x${bytesToHex(spendingPrivateKey)}`, viewingPrivateKey: `0x${bytesToHex(viewingPrivateKey)}`, metaAddress: formatMetaAddress({ spendingPublicKey: `0x${bytesToHex(secp.getPublicKey(spendingPrivateKey, false))}`, viewingPublicKey: `0x${bytesToHex(secp.getPublicKey(viewingPrivateKey, false))}` }) };
}
...
export function makeStealthPayment(meta: MetaAddress) {
  ...
  return { stealthAddress: `0x${keccak256(hexToBytes(pointHex(recipient))).slice(-40)}` as `0x${string}`, ephemeralPubKey: `0x${bytesToHex(secp.getPublicKey(ephemeralPrivateKey, false))}` as `0x${string}` };
}
...
export function publicKeyFromPrivate(privateKey: string) { return `0x${bytesToHex(secp.getPublicKey(hexToBytes(asHex(privateKey)), false))}`; }
